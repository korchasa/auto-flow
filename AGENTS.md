...
- **Plugin payload cross-repo sync (FR-E70/E72/E74):** The engine ships
  ...
  **Self-contained runtime (FR-E74).** The payload also ships a
  ...
  unset).
- **Plugin agents/skills layout.** The plugin's interactive surface
  lives in two parallel trees under
  `claude-plugin/plugins/flowai-workflow/`. Source of truth — never
...
- `CHANGELOG.md` — immutable release log (regenerated by
  `standard-version`; rewriting historical entries breaks the link to
  git tags).
