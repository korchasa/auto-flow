...
- **Description:** Plan critique and revision functionality is now absorbed into the Tech Lead agent (FR-S5). The Tech Lead critiques the Architect's plan, selects a variant, and produces the task breakdown — all in one stage. Separate reviewer agent eliminated (FR-S15).
  ...
  Previous input/output (pre-FR-S15): `02-plan.md` → `03-revised-plan.md` (no longer produced as separate artifact).
- **Supersedes:** FR-S4 (absorbed by FR-S5/FR-S15).
- **Acceptance criteria:**
...
- **Description:** SDS update functionality is now absorbed into the Tech Lead agent (FR-S5). The Tech Lead updates `documents/design-sdlc.md` as part of its decision-making stage. Separate sds-update agent eliminated (FR-S15).
  ...
  Previous input/output (pre-FR-S15): `03-decision.md` → updated `documents/design-sdlc.md` (now done by Tech Lead).
- **Supersedes:** FR-S6 (absorbed by FR-S5/FR-S15).
- **Acceptance criteria:**
...
- **Description:** The Developer and QA agents work as an iterative pair. Developer implements, QA verifies. If QA finds issues, Developer fixes them. The loop continues until QA passes or the iteration limit is reached.
  ...
  **Developer artefact contract:**
  - Input: `03-decision.md`, `documents/requirements-sdlc.md`, `documents/design-sdlc.md`, source code. On subsequent iterations: previous QA report (`05-qa-report.md`).
  - Output: code changes, tests, commits and pushes on feature branch. PR comment with implementation summary.

  **QA artefact contract:**
  - Input: `01-spec.md`, `03-decision.md`, all changed files, test results.
  - Output: `05-qa-report.md` in node output directory. PR review verdict (`gh pr review`: approve/request-changes).
...
- **Description:** The Presenter agent (pre-FR-S15) created human-readable
  change summary for the PR description. Functionality is now covered by:
  Tech Lead (draft PR creation with implementation plan summary), Developer
  (PR comments with implementation progress, FR-S7), and Tech Lead Review
  (final review, CI gate, merge decision).
- **Status:** Absorbed into Tech Lead (FR-S5) and Tech Lead Review (FR-S15).
  No separate Presenter agent.
- **Supersedes:** FR-S8 (absorbed by FR-S5/FR-S15).
