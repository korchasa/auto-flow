...
> Статус «in progress» отражает три пункта готовности, которые остались
> невыполненными в том виде, как были записаны: снимок окна в PNG, флаг
> `--project` и отдельная «фразовая книга». Приложение при этом сделано
> целиком и работает — что именно разошлось с планом и почему, описано ниже.
...
### Что закрылось уже после первой сдачи

Живая проверка нашла ещё два пробела, оба закрыты:

- Снаружи нельзя было задать путь к движку, поэтому внешний агент открывал
  проект и читал прогоны, но никогда не доводил приложение до состояния, где
  работает управление. Появилась команда `set_engine_path`. Копайлоту её не
  дают вовсе — это отдельное поле в реестре, а не третьи ворота: ворота
  работают, когда человек может судить о просьбе по одной строке, а «запусти
  вот эту программу» так не судят.
- Панель разговора запускает отдельную программу, и её нельзя было проверить
  ничем, кроме набора текста в окне. Появился маршрут `POST /copilot`. Своим
  токеном копайлот до него не достаёт.

Копайлот проверен на настоящем `claude` и настоящих прогонах этого репозитория:
он вызвал `list_runs` и `get_run` и верно описал, на чём встал прогон, а на
просьбу запустить новый получил «ждём согласия человека», сказал об этом и не
стал повторять.
