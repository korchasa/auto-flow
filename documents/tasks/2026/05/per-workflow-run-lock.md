...
date: "2026-05-01"
status: done
implements: [FR-E25, FR-E53, FR-E54, FR-E57, FR-S47]
tags: [decision, engine, lock, accepted]
related_tasks:
...
Align lock scope with the actual isolation boundary (workflow folder) so
multi-workflow repos can run dogfood variants in parallel without
falsely-serialized PID locks.
...
- Lock path is purely a function of `workflowDir`.
- No fallback to the legacy global path.
- PID-liveness and stale-reclaim semantics carry over from FR-E25
  unchanged.
- Tests in `lock_test.ts` cover both same-folder serialization and
  distinct-folder concurrency.

## Definition of Done

- [x] `defaultLockPath(workflowDir)` returns
      `<workflowDir>/runs/.lock`.
- [x] Engine derives `workflowDir` once via
      `deriveWorkflowDir(options.config_path)` and threads it into
      `acquireLock` / `releaseLock`.
- [x] Two runs against the same workflow folder still serialize
      (FR-E25 invariants — PID liveness check, stale-lock reclaim on
      dead PID — preserved bit-for-bit).
- [x] Two runs against different workflow folders proceed in parallel.
- [x] `EngineOptions.lock_path` test-only override still wins when set.
- [x] Legacy `.flowai-workflow/runs/.lock` from older binaries is
      ignored as an orphan file.
...
- **Negative.** Stale repo-global lock files in long-lived checkouts
  ...
  time (that was `2026/05/per-run-worktree-co-location.md` /
  FR-E57's job) — the FR-E54 change alone could have allowed
  cross-workflow worktree collisions; FR-E57 closes the gap.
