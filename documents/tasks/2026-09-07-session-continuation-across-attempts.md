...
- [x] FR-E100 — The dogfood `github-inbox` and `github-inbox-opencode` loops opt the Developer node in, and a live `github-inbox` (claude) run shows iteration 2 resuming iteration 1's session. Test: `manual — korchasa`. Evidence: run `20260907T032306` — `grep -o '"session_id":"[^"]*"' .flowai-workflow/github-inbox/runs/20260907T032306/journal.jsonl` (build iteration 1 recorded `019da1fc-…`), run log line `[04:03:24] build session: continuing build`, and `build/stream.log` replaying iteration 1's turns at `04:03:26`. Three pre-existing claude/ACP defects blocked the run first and were fixed in `3f46e90`, `50315e8`, `0fee16f`.
- [ ] FR-E100 — A live `github-inbox-opencode` run shows iteration 2 resuming iteration 1's session. Test: `manual — korchasa`. Evidence: pending — run `20260907T032722` recorded `build`'s session (`ses_…`) but QA passed on iteration 1, so no second iteration ran.
